...
void main() {
    ...
    if(handDepth != 1.0) handDepth = 0.0;
    ...
    if(texture(u_geometry_depth_particles, texcoord).r == 1.0 && handDepth != 0.0 && frx_worldIsOverworld == 1) color.rgb = mix(color.rgb, mix(color.rgb, color.rgb * vec3(1.2 * cloudNoise), cloudNoise), frx_smootherstep(0.0, 0.3, viewSpacePos.y));
